    ...
    for (int point_index = 1; point_index < point_count; ++point_index)
    {
        ...
        while (true)
        {
            ...
            if (cacheIt == cache.end())
            {
                ...
                const auto lengthNew   = cubic_bezier_length(front.p0, front.p1, front.p2, front.p3);

                cacheIt = cache.emplace(t, lengthNew).first;
            }

            const auto lengthNew   = cacheIt->second;
            const auto error    = targetLength - lengthNew;
