...
public class ExportKey
{
    ...
    private static void ExportLightActivations(ref List<ActivationLightData> collection, Key key)
    {
        ...
        {
            if(behavior.IsNullPrintWarning(behavior.target, "Fix this by adding a Target =) H�lsningar, Axel Savage"))
                return;
            if (behavior.IsNullPrintWarning(behavior.targetLight, "Fix this by adding a Target Light =) H�lsningar, Axel Savage"))
                return;
